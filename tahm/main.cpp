...
void start()
{
    ...
    mediumFont = tahm.graphics.font->newFont("../tahm/fonts/PublicPixel.ttf", 16);
    scoreFont = tahm.graphics.font->newFont("../tahm/fonts/PublicPixel.ttf", 84);

	// Sound setup
	sounds.hit = tahm.audio.newSound("../tahm/sounds/hit.wav");
	sounds.score = tahm.audio.newSound("../tahm/sounds/score.wav");
...
void draw()
{
	...
	if (gameState == START)
	{
		tahm.graphics.renderText("center", WINDOW_WIDTH, 0, 50, mediumFont, "WELCOME TO PONG++ CREATED BY TAMTA ASATIANI");
		tahm.graphics.renderText("center", WINDOW_WIDTH, 0, 80, mediumFont, "PRESS ENTER TO START PLAYING!");
	}
	else if (gameState == SERVE)
	{
		tahm.graphics.renderText("center", WINDOW_WIDTH, 0, 50, mediumFont, "PRESS ENTER TO SERVE");
	}
	else if (gameState == WIN)
	{
		tahm.graphics.renderText("center", WINDOW_WIDTH, 0, 50, mediumFont, generateWinText().c_str());
		tahm.graphics.renderText("center", WINDOW_WIDTH, 0, 80, mediumFont, "PRESS ENTER TO TRY AGAIN");
	}

	// score rendering
	tahm.graphics.renderText("left", WINDOW_WIDTH, 500, WINDOW_HEIGHT / 3, scoreFont, player1ScoreText.c_str());
	tahm.graphics.renderText("right", WINDOW_WIDTH, -500, WINDOW_HEIGHT / 3, scoreFont, player2ScoreText.c_str());
